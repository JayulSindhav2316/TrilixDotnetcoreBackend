using Max.Reporting.API.Extensions;
using Max.Reporting.Infrastructure;
using Max.Reporting.Infrastructure.Persistence;
using Max.Reporting.Application;
...
using Max.Reporting.Infrastructure.MySqlStorage;
using Microsoft.Extensions.Logging;
using Newtonsoft.Json;
using Max.Reporting.API.Middlewares;
using System.IO;
using Microsoft.AspNetCore.DataProtection;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.Caching.Memory;
using Max.Core.DataProtection;
...
builder.Services.AddSingleton<IDataProtectorExtention, DataProtectionExtention>();
builder.Services.AddDataProtection()
    .SetApplicationName("MemberMax")
    .PersistKeysToFileSystem(new DirectoryInfo(Path.Combine(builder.Configuration["AppSettings:DataProtectionPath"], "dp-keys")));
...
builder.Services.TryAddSingleton<IReportServiceConfiguration>(sp =>
    new ReportServiceConfiguration
    {
        ...
        HostAppId = "Max.Reporting.API",
        Storage = new MySqlServerStorage(sp.GetService<IConfiguration>()),
        ReportSourceResolver = new TypeReportSourceResolver()
                                    .AddFallbackResolver(
                                        new MySqlReportSourceResolver(builder.Configuration,sp.GetService<IMemoryCache>(),sp.GetService<IDataProtectorExtention>()))
    });
...
{
    DefinitionStorage = new MySqlDefinitionStorage(reportsPath, sp.GetService<IConfiguration>()),    
    SettingsStorage = new MySqlSettingsStorage(sp.GetService<IConfiguration>()),
});
...
app.MigrateDatabase<ReportContext>((context, services) =>
{
