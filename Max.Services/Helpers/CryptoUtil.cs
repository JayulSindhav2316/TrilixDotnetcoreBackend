    ...
    public class CryptoUtil
    {
        ...
        public static void EncryptFile(string inFile, string outFile, string password)
        {
                      ...
                      fout = File.OpenWrite(outFile))
            {
                ...
                SymmetricAlgorithm sma = CryptoUtil.CreateRijndael(password, salt);
                sma.IV = IV;
        ...
        public static int DecryptFile(string inFile, string outFile, string password)
        {
                      ...
                      fout = File.OpenWrite(outFile))
            {
                          ...
                          chash = new CryptoStream(Stream.Null, hasher, CryptoStreamMode.Write))
                {
                    ...
                    // .net 6.0 hack : If bytesLeft are < 16 then it no longer returns the last chunk
                    // Ref: https://learn.microsoft.com/en-us/dotnet/core/compatibility/core-libraries/6.0/partial-byte-reads-in-streams
                    while (bytesLeft > 0)
                    {
                        read = cin.Read(bytes, 0, (int)bytesLeft);
                        if (read > 0)
                        {
                            fout.Write(bytes, 0, read);
                            chash.Write(bytes, 0, read);
                ...
                if (outValue != lSize)
                {
                    return (int)FileDownloadStatus.FileSizeMismatch;
                }
            }
            return (int)FileDownloadStatus.Successful;
        }
    }
}
